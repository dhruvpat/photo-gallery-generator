...
class HomeController extends BaseController {
	...
	*/
	...
	public function index()
	{
		$page = Input::get('p'); 
		if($page !='') :
			$content = Pages::where('alias','=',$page);
			if($content->count() >=1)
			{
				$content = $content->get();
				$row = $content[0];
				...
				{	
					...
					{
						return Redirect::to('')
							->with('message', SiteHelpers::alert('error',' You do dont have access to view the page ! '));				
					}
				}				
				...
				if(file_exists($filename))
				{
					...
					return Redirect::to('')
						->with('message', SiteHelpers::alert('error',' Page does not exist ! '));					
				}
