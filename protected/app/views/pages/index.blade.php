	  ...
	  <ul class="breadcrumb-buttons collapse">
        <li class="dropdown"><a href="" class="dropdown-toggle" data-toggle="dropdown"><i class="icon-search3"></i> <span>Search</span> <b class="caret"></b></a>
          <div class="popup dropdown-menu dropdown-menu-right">
			...
			<li><a href="{{ URL::to('pages/download') }}" class="tips" title="Download"><i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/pages') }}" class="tips"  title="Configuration"><i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('pages/add') }}" class="tips"  title="Create"><i class="icon-plus-circle2"></i>&nbsp;</a></li>
			@endif  
        ...
        <tbody>
            @foreach ($rowData as $row)
				...
				<ul style="display: none;" class="dropdown-menu icons-right">
					...
					<li><a href="{{ URL::to('pages/show/'.$id)}}"><i class="icon-cogs"></i> View Detail </a></li>
					@endif
					@if($access['is_edit'] ==1)
					<li><a href="{{ URL::to('pages/add/'.$id)}}"><i class="icon-grid3"></i> Edit</a></li>
					@endif
				 ...
				 @endforeach
				 <td > <a href="{{ URL::to('?p='.$row->alias) }} " target="_blank"> <code class="text-mute">{{ URL::to('?p='.$row->alias) }}</code> </a> </td>		
                </tr>
            @endforeach
	...
	<div class="row">
	 <div class="col-sm-5">
	  <div class="table-actions">
	   ...
	   {{ Form::open(array('url'=>'pages/filter/')) }}
