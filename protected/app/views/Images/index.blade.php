	  ...
	  <ul class="breadcrumb-buttons collapse">
        <li class="dropdown"><a href="" class="dropdown-toggle" data-toggle="dropdown"><i class="icon-search3"></i> <span>Search</span> <b class="caret"></b></a>
          <div class="popup dropdown-menu dropdown-menu-right">
			...
			<li><a href="{{ URL::to('Images/download') }}" class="tips" title="Download"><i class="icon-folder-download2"></i>&nbsp; </a></li>	
			@endif		
		 	@if(Session::get('gid') ==1)
			<li><a href="{{ URL::to('module/config/Images') }}" class="tips"  title="Configuration"><i class="icon-cog"></i>&nbsp; </a></li>	
			@endif  
			@if($access['is_add'] ==1)
	   		<li><a href="{{ URL::to('Images/add') }}" class="tips"  title="Create"><i class="icon-plus-circle2"></i>&nbsp;</a></li>
			@endif  
    ...
    <table class="table table-striped ">
        ...
        <tbody>
            @foreach ($rowData as $row)
                <tr>
					<td ><img src="<?php echo Config::get('general.image_folder_path_web'); ?>{{ $row->keyword }}/{{ $row->filename }}-md.{{ $row->extension }}" style="max-width: 400px; max-height: 400px;"> </td>
					<td width="50"> {{ ++$i }} </td>
				...
				<ul style="display: none;" class="dropdown-menu icons-right">
					...
					<li><a href="{{ URL::to('Images/show/'.$id)}}"><i class="icon-cogs"></i> View Detail </a></li>
					@endif
					@if($access['is_edit'] ==1)
					<li><a href="{{ URL::to('Images/add/'.$id)}}"><i class="icon-grid3"></i> Edit</a></li>
					@endif
	...
	<div class="row">
	 <div class="col-sm-5">
	  <div class="table-actions">
	   ...
	   {{ Form::open(array('url'=>'Images/filter/')) }}
